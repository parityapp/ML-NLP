...
#        from nltk.corpus import brown as corpus
        ...
        # return [corpus.sents("trial.txt")]
        # sentence_tokenizer = nltk.tokenize.punkt.PunktSentenceTokenizer()
        # sentence = sentence_tokenizer.tokenize(text)

        # from summarization import load_sample_text
        # text=load_sample_text(lines=2)
        # return [nltk.word_tokenize(t) for t in nltk.sent_tokenize(text)]'''
        # return [each_sent for each_sent in sentence]
    ...
    def doc_to_ids(self, doc):
        ...
        for term in doc:
            ...
            if word_id is not None:
                term_list.append(word_id)
                # if not words.has_key(id):
                if word_id not in words:
                    words[word_id] = 1
                    self.docfreq[word_id] += 1
        if "close" in dir(doc):
            doc.close()
        return term_list
